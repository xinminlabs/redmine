  ...
  def find_current_user
    ...
    unless api_request?
      if session[:user_id]
        ...
        user = User.where(rss_key: params[:key]).first
      end
    end
    if user.nil? && Setting.rest_api_enabled? && accept_api_auth?
      if (key = api_key_from_request)
        ...
        authenticate_with_http_basic do |username, password|
          user = User.try_to_login(username, password) || User.where(api_key: username).first
        end
      ...
      if user && user.admin? && (username = api_switch_user_from_request)
        su = User.where(login: username).first
        if su && su.active?
  ...
  def find_attachments
    if (attachments = params[:attachments]).present?
      att = attachments.values.collect do |attachment|
        Attachment.where(token: attachment[:token]).first if attachment[:token].present?
      end
      att.compact!
    end
    @attachments = att || []
